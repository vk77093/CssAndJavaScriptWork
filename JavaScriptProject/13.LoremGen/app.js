...
let resultCount=document.querySelector('.result')




form.addEventListener('submit',(e)=>{
  //it will directly going to submit to server
  //so we need to chnage the default behaviour of the form

  e.preventDefault();
  //console.log('hello vijay');
  let values=amount.value;

  //console.log(typeof values); // it will provides the string
  // so that we need to parse it into integer
 let neeInt= parseInt(values);
  //console.log(typeof neeInt); // number in blue and string in black color
  let randomPar=Math.floor(Math.random()*text.length)
  //console.log(randomPar);

  if(isNaN(neeInt)||neeInt< 0 ||neeInt> 9){
    loremText.innerHTML=`<p class="result">${text[randomPar]}</p>`;
  }else{
    let tempData=text.slice(0,neeInt);
    resultCount.innerHTML=`You gotted the ${neeInt} paragraph`;
   // loremText.innerHTML=tempData;
